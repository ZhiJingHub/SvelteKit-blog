...
<Card>
	...
	<CardContent class="space-y-4">
		...
		<div class="space-y-2"><Label>图标圆角: {iconRadius}%</Label><Slider value={[iconRadius]} onValueChange={(v) => (iconRadius = v[0])} min={0} max={50} /></div>
		<div class="space-y-2"><Label>间距: {gap}px</Label><Slider value={[gap]} onValueChange={(v) => (gap = v[0])} min={0} max={200} /></div>
	</CardContent>
</Card>
