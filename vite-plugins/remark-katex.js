...
function renderFormula(formula, displayMode) {
	try {
		const html = katex.renderToString(formula, { displayMode, throwOnError: false, output: 'html', strict: false });
	...
	return (tree) => {
		...
		while (i < tree.children.length) {
			...
			if (node.type === 'paragraph' && node.children) {
				// Check for inline $$...$$ within a single paragraph
			...
			// Check for multi-paragraph $$ blocks
